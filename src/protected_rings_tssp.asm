...
switchToRing3FLATuserLow:
	...
	or     edx,0xE0000              ; Fix flat instruction address
	push   dword edx                ; push return EIP
	iretd
...
switchToRing3V86_0:
	...
	push   dword 0xE000             ; push user code segment with RPL=3
	push   dword edx                ; push return EIP
	iretd
...
testCallGateWithParameters:
	...
	jmp    switchToRing3low         ; Switch to ring 3 to start the test

errorTSS:
	; If we end up here, the test failed. Loop forever.
	cli
	hlt
...
switchToRing0_2:
	...
	mov    edi, .ring0_3
	mov    dx,  ACC_DPL_3|0xA    ; the DPL needs to be 3
	call   CU_SEG_PROT32:initCallGate286Far
...
.ring0_2: ;32-bit call gate entry point
	...
	mov    bp, ss
	cmp    bx, bp
	jnz    errorTSS                ; Invalid kernel stack
	pop    ebx
	pop    ebp
	pop    ds
	push   eax
